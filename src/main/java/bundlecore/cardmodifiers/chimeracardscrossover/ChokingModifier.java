...
/**
 * Card cost increases, card now applies choke.
 */
public class ChokingModifier extends AbstractAugment implements DynvarCarrier {
    public static final String ID = BundleCoreMain.makeID(ChokingModifier.class.getSimpleName());
    ...
    public boolean validCard(AbstractCard card) {
        return card.cost >= 0 && card.baseDamage >= 2 && card.type == AbstractCard.CardType.ATTACK && cardCheck(card, (c) -> usesEnemyTargeting());
    }

    @Override
    public AbstractCardModifier makeCopy() {
        return new ChokingModifier();
    }

    @Override
    public void onUse (AbstractCard card, AbstractCreature target, UseCardAction action){
        GFL.atb(new ApplyPowerAction(target, GFL.GAP(), new ChokePower(target, getBaseVal(card)), getBaseVal(card)));
    }
